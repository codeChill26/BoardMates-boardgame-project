export const translations = {
  vi: {
    navbar: {
      home: 'Trang chủ',
      community: 'Cộng đồng',
      events: 'Sự kiện',
      joinUs: 'Tham gia',
      about: 'Giới thiệu',
      comingSoon: 'Sắp ra mắt',
      games: 'Trò chơi',
      news: 'Tin tức',
      shop: 'Cửa hàng',
      logout: 'Đăng xuất',
      login: 'Đăng nhập',
      profile: 'Profile',
      gameVault: 'Kho Game',
    },
    // Tour dan khach lan dau: key trong `steps` phai trung `key` cua STEPS
    // trong components/common/NavbarTour.jsx.
    tour: {
      titleBar: 'huong_dan.exe',
      step: 'Bước',
      skip: 'Bỏ qua',
      back: 'Quay lại',
      next: 'Tiếp',
      done: 'Bắt đầu khám phá',
      // Chi hien tren man hinh hep, khi muc duoc noi toi dang nam trong menu.
      inMenu: 'Nằm trong menu ☰',
      steps: {
        welcome: {
          title: 'Chào mừng tới BoardMates',
          body: 'Có vẻ đây là lần đầu bạn ghé qua. Để mình dẫn bạn đi một vòng thanh điều hướng, chỉ mất khoảng 30 giây.',
        },
        home: {
          title: 'Trang chủ',
          body: 'Điểm bắt đầu: BoardMates là gì và chúng mình đang xây dựng điều gì.',
        },
        community: {
          title: 'Cộng đồng',
          body: 'Nơi bạn tìm thấy những người cùng gu board game để chơi và chia sẻ. Phần này đang được xây, sẽ mở sớm.',
        },
        events: {
          title: 'Sự kiện',
          body: 'Board Game Night, workshop, giải đấu. Lịch sự kiện và đăng ký tham gia sẽ nằm ở đây.',
        },
        joinUs: {
          title: 'Tham gia',
          body: 'Chúng mình đang tuyển Core Team. Xem mô tả công việc của từng ban và nộp đơn ứng tuyển ngay tại đây.',
        },
        about: {
          title: 'Giới thiệu',
          body: 'Tầm nhìn, triết lý và lộ trình của BoardMates — kể qua sáu mặt xúc xắc. Cuối trang còn phần giới thiệu Youth+, đơn vị chủ quản của tụi mình.',
        },
        language: {
          title: 'Đổi ngôn ngữ',
          body: 'Chuyển qua lại giữa Tiếng Việt và English bất cứ lúc nào.',
        },
      },
    },
    footer: {
      brand: 'BoardMates',
      copyright: '© 2026 BoardMates. Tất cả quyền được bảo lưu.',
      rights: 'Bảo lưu mọi quyền.',
    },
    comingSoon: {
      badge: 'Sắp ra mắt',
      community: {
        title: 'Cộng đồng',
        desc: 'Nơi bạn tìm thấy những người cùng sở thích, chia sẻ và cùng nhau phát triển qua board game. Chúng mình đang xây dựng phần này.',
      },
      events: {
        title: 'Sự kiện',
        desc: 'Board Game Night, Workshop, Networking, Tournament. Lịch sự kiện và đăng ký tham gia sẽ có ở đây.',
      },
      cta: 'Muốn cùng xây dựng? Tham gia Core Team',
    },
    about: {
      title: 'Giới thiệu',
      intro: 'Đổ xúc xắc để mở từng phần câu chuyện của BoardMates.',
      diceHint: 'Bấm để đổ',
      diceRolling: 'Đang đổ...',
      of: 'trên',
      // Khoi gioi thieu don vi chu quan, nam duoi cua so about.exe.
      youth: {
        body: 'Youth+ là hệ sinh thái giáo dục và cộng đồng phi lợi nhuận dành cho người trẻ Việt Nam, với sứ mệnh kết nối, định hướng và tạo cơ hội để giới trẻ phát triển bản thân thông qua học tập, trải nghiệm và các dự án cộng đồng.',
        modelLabel: 'Mô hình kết nối',
        modelNodes: ['Bạn trẻ', 'Bạn trẻ', 'Mentor', 'Doanh nghiệp'],
        modelNote: 'Tạo môi trường để người trẻ học hỏi và cùng tạo ra những giá trị tích cực.',
        stats: [
          { value: '63K+', name: 'Người theo dõi Facebook' },
          { value: '10+', name: 'Năm hoạt động' },
        ],
        cta: 'Xem fanpage Youth+',
        ctaPending: 'Link fanpage sẽ được cập nhật',
      },
      // 6 mat xuc xac = 6 muc noi dung. Moi muc mot "kind" rieng vi cach trinh bay khac nhau.
      sections: [
        {
          face: 1,
          kind: 'prose',
          label: 'Tầm nhìn',
          body: 'BoardMates là dự án thuộc YOUTH + COMMUNITY, là nền tảng cộng đồng kết nối và sáng tạo thông qua board game. Board game chỉ là điểm khởi đầu. Điều BoardMates muốn xây dựng là một hệ sinh thái nơi các người trẻ có thể gặp gỡ, chia sẻ, học hỏi, hợp tác và cùng nhau phát triển trong thời đại số hiện nay.',
        },
        {
          face: 2,
          kind: 'lines',
          label: 'Triết lý',
          lines: [
            'Board game là công cụ.',
            'Con người là trung tâm.',
            'Kết nối là giá trị.',
            'Sáng tạo là mục tiêu.',
          ],
          closing: 'BoardMates tồn tại để tạo nên những kết nối có ý nghĩa thông qua board game.',
        },
        {
          face: 3,
          kind: 'chain',
          label: 'Hệ sinh thái',
          body: 'BoardMates không chỉ là một cộng đồng. Chúng mình đang xây dựng một hệ sinh thái.',
          nodes: [
            'Câu lạc bộ Đại học',
            'Board Game Café',
            'Cửa hàng',
            'Nhà xuất bản',
            'Nhà sáng tạo',
            'Người chơi',
          ],
        },
        {
          face: 4,
          kind: 'roadmap',
          label: 'Lộ trình',
          phases: [
            { phase: 'Phase 1', name: 'Cộng đồng', percent: 50 },
            { phase: 'Phase 2', name: 'Sự kiện', percent: 30 },
            { phase: 'Phase 3', name: 'Nền tảng', percent: 20 },
            { phase: 'Phase 4', name: 'Văn hóa', percent: 50 },
          ],
        },
        {
          face: 5,
          kind: 'groups',
          label: 'Mục tiêu',
          groups: [
            {
              name: 'Ngắn hạn',
              items: [
                'Giới thiệu BoardMates',
                'Xây dựng thương hiệu',
                'Tuyển Core Team',
                'Thu hút cộng đồng đầu tiên',
                'Tổ chức sự kiện đầu tiên',
              ],
            },
            {
              name: 'Trung hạn',
              items: [
                'Quản lý cộng đồng',
                'Quản lý sự kiện',
                'Blog',
                'Resource Center',
                'Partner Network',
              ],
            },
            {
              name: 'Dài hạn',
              items: [
                'Marketplace',
                'Mobile App',
                'Community Hub',
                'Board Game Database',
                'Member Profile',
                'Gamification',
              ],
            },
          ],
        },
        {
          face: 6,
          kind: 'metrics',
          label: 'Thước đo thành công',
          metrics: [
            { value: '1.000', name: 'Thành viên cộng đồng' },
            { value: '10', name: 'Thành viên Core Team' },
            { value: '20', name: 'Sự kiện' },
            { value: '50', name: 'Đối tác' },
            { value: '100', name: 'Tin đăng Marketplace' },
            { value: '10.000', name: 'Lượt truy cập mỗi tháng' },
          ],
        },
      ],
    },
    joinUs: {
      title: 'Tham gia cùng chúng mình',
      intro: 'BoardMates vẫn đang trong giai đoạn đầu. Chúng mình đang tìm kiếm những người muốn xây dựng cộng đồng từ con số 0.',
      openPositions: 'Vị trí đang mở',
      headcount: 'Số lượng',
      headcountUnit: 'người',
      mission: 'Sứ mệnh',
      responsibilities: 'Công việc',
      requirements: 'Yêu cầu',
      benefits: 'Quyền lợi',
      growthPath: 'Lộ trình phát triển',
      applyTitle: 'Thấy mình phù hợp?',
      applyDesc: 'Chúng mình quan tâm tới tinh thần xây dựng hơn là kinh nghiệm. Nếu bạn muốn cùng làm từ con số 0, hãy nói chuyện với chúng mình.',
      applyCta: 'Ứng tuyển',
      applyPending: 'Kênh ứng tuyển sẽ sớm được mở',
    },
    hero: {
      welcome: 'Chào mừng đến với',
      subtitle: 'Kết nối sinh viên, lan tỏa văn hóa board game và tạo nên những trải nghiệm thật ngoài đời thực.',
      explore: 'Khám phá ngay',
      quickLinks: 'Liên kết nhanh',
      featured: 'Xây dựng cộng đồng - nơi mọi người có thể dừng chân, kết nối và học hỏi lẫn nhau qua Board Games trong thời đại số.',
      collector: 'Sứ mệnh',
    },
    home: {
      blog: {
        latest: 'Tin tức mới nhất',
        title: 'Từ Blog',
        desc: 'Chúng tôi không chỉ lưu trữ trò chơi, chúng tôi kể những câu chuyện đằng sau chúng. Theo dõi những bài viết mới nhất về văn hóa, lịch sử và thiết kế board game.',
        viewAll: 'Xem tất cả bài viết',
        readMore: 'Đọc tiếp',
      },
      games: {
        newCollection: 'Bộ sưu tập mới',
        title: 'Dự án mới',
        viewMore: 'Xem thêm',
        players: 'Người',
      },
      links: {
        blog: 'Blog',
        projects: 'Dự án',
        info: 'Thông tin',
        contact: 'Liên hệ',
      }
    },
    auth: {
      welcomeBack: 'Chào mừng bạn trở lại',
      startJourney: 'Bắt đầu hành trình của bạn',
      fullName: 'Họ và tên',
      email: 'Email',
      password: 'Mật khẩu',
      confirmPassword: 'Nhập lại mật khẩu',
      forgotPassword: 'Quên mật khẩu?',
      login: 'ĐĂNG NHẬP',
      register: 'ĐĂNG KÝ',
      noAccount: 'Chưa có tài khoản?',
      haveAccount: 'Đã có tài khoản?',
      registerNow: 'Đăng ký ngay',
      loginNow: 'Đăng nhập',
      agreeTerms: 'Tôi đồng ý với điều khoản và chính sách bảo mật.',
      processing: 'Đang xử lý...',
      authenticating: 'Đang xác thực...',
      mockRegisterSuccess: 'Đăng ký giả lập thành công! Đang chuyển hướng...',
    },
    marketplace: {
      collection: 'BỘ SƯU TẬP 2024',
      title: 'Cửa hàng',
      addGame: '+ Đăng trò chơi',
      sortBy: 'Sắp xếp:',
      newest: 'Mới nhất',
      categories: 'Danh mục',
      price: 'Mức giá',
      updating: 'ĐANG CẬP NHẬT',
      maintenance: 'Hệ thống đang bảo trì dữ liệu',
      underMaintenance: 'Tính năng đang bảo trì',
      memberOffer: 'Ưu đãi thành viên',
      memberDesc: 'Giảm ngay 10% cho đơn hàng đầu tiên của bạn.',
      joinNow: 'Tham gia ngay',
    },
    profile: {
      myProfile: 'Hồ sơ của tôi',
      editProfile: 'Chỉnh sửa hồ sơ',
      listings: 'Tin đăng của tôi',
      rentals: 'Lịch sử thuê',
      purchases: 'Đơn mua',
      noListings: 'Bạn chưa có tin đăng nào.',
      since: 'Thành viên từ',
      rating: 'Đánh giá',
      addNew: '+ Thêm mới',
      view: 'Xem',
      edit: 'Sửa',
      remove: 'Xóa',
      info: 'Thông tin cá nhân',
      phone: 'Số điện thoại',
      address: 'Địa chỉ',
      orders: {
        title: 'Lịch sử đơn hàng',
        viewAll: 'Xem tất cả',
        id: 'Mã đơn',
        date: 'Ngày mua',
        status: 'Trạng thái',
        total: 'Tổng tiền',
        completed: 'Hoàn tất',
        cancelled: 'Đã hủy',
      },
      favorites: {
        title: 'Trò chơi yêu thích',
      }
    },
    modal: {
      addNew: 'Thêm Boardgame mới',
      title: 'Tên trò chơi',
      players: 'Số người chơi',
      gameplay: 'Thể loại',
      description: 'Mô tả',
      priceSell: 'Giá bán (VND)',
      priceRent: 'Giá thuê (VND)',
      condition: 'Tình trạng',
      type: 'Hình thức',
      imageUrl: 'Hình ảnh',
      cancel: 'Hủy',
      submit: 'Đăng tin',
      submitting: 'Đang gửi...',
      mockSuccess: 'Đăng tin thành công (Giả lập)',
      maintenanceMsg: 'Tính năng đang bảo trì dữ liệu. Vui lòng quay lại sau!',
    },
    events: {
      badge: 'Cộng đồng Board Game',
      title: 'Sự kiện & Lên Kèo Chơi',
      subtitle: 'Tìm bạn bè cùng gu, tham gia các buổi offline tụ tập, workshop chiến thuật và giải đấu đỉnh cao.',
      createBtn: '+ Lên Kèo / Tạo Sự Kiện',
      searchPlaceholder: 'Tìm kèo theo tên game, địa điểm, từ khóa...',
      tabs: {
        all: 'Tất cả sự kiện',
        myHosted: 'Kèo tôi tạo',
        myJoined: 'Kèo tôi tham gia',
      },
      filterType: 'Loại sự kiện',
      filterCity: 'Khu vực',
      filterStatus: 'Trạng thái',
      filterRadius: 'Bán kính GPS',
      gpsNearMe: '📍 Gần tôi (GPS)',
      gpsLocating: 'Đang định vị...',
      gpsActive: 'Đã bật GPS',
      gpsDistance: 'Cách {dist} km',
      radiusAll: 'Tất cả khoảng cách',
      radius5: '< 5 km',
      radius10: '< 10 km',
      radius25: '< 25 km',
      radius50: '< 50 km',
      allTypes: 'Tất cả loại kèo',
      allCities: 'Tất cả khu vực',
      allStatuses: 'Tất cả trạng thái',
      types: {
        CASUAL: 'Lên kèo giao lưu',
        TOURNAMENT: 'Giải đấu',
        WORKSHOP: 'Workshop / Học chơi',
        NIGHT: 'Board Game Night',
      },
      statuses: {
        OPEN: 'Đang mở slot',
        FULL: 'Đã đủ người',
        ONGOING: 'Đang diễn ra',
        COMPLETED: 'Đã kết thúc',
        CANCELLED: 'Đã hủy',
      },
      skillLevels: {
        ALL: 'Mọi trình độ',
        BEGINNER: 'Nhập môn / Người mới',
        INTERMEDIATE: 'Có kinh nghiệm',
        EXPERT: 'Cao thủ / Chuyên sâu',
      },
      card: {
        host: 'Host:',
        slots: 'Slot:',
        full: 'Đã hết chỗ',
        fee: 'Chi phí:',
        free: 'Miễn phí / Tự túc nước',
        joinBtn: 'Tham gia kèo',
        joinedBtn: 'Đã tham gia',
        viewDetails: 'Xem chi tiết',
        spotsLeft: 'còn {count} chỗ',
        away: 'Cách bạn {dist} km',
      },
      modal: {
        createTitle: 'Lên Kèo / Tạo Sự Kiện Mới',
        titleLabel: 'Tiêu đề sự kiện / Tên kèo',
        titlePlaceholder: 'Vd: Kèo Catan cuối tuần chill, Workshop Scythe cho newbie...',
        typeLabel: 'Hình thức kèo',
        gameLabel: 'Tên Board Game',
        gamePlaceholder: 'Vd: Catan, Splendor, Root, Dune: Imperium...',
        imageLabel: 'Link ảnh minh họa (URL)',
        imagePlaceholder: 'https://images.unsplash.com/...',
        dateLabel: 'Thời gian bắt đầu',
        endDateLabel: 'Thời gian kết thúc (Tùy chọn)',
        locationLabel: 'Tên quán / Địa điểm',
        locationPlaceholder: 'Vd: The Mind Cafe, Cube Cafe, Nhà riêng...',
        addressLabel: 'Địa chỉ cụ thể',
        addressPlaceholder: 'Vd: 284/41 Lý Thường Kiệt, P.14, Quận 10...',
        cityLabel: 'Khu vực / Thành phố',
        maxParticipantsLabel: 'Số người tối đa (bao gồm Host)',
        feeLabel: 'Chi phí tham gia (VNĐ)',
        feeHint: 'Nhập 0 nếu Miễn phí hoặc Tự túc chia tiền nước',
        skillLevelLabel: 'Yêu cầu trình độ',
        descLabel: 'Mô tả & Ghi chú luật chơi',
        descPlaceholder: 'Ghi chú về thể lệ, luật chơi, ai mang theo game, hướng dẫn chơi cho bạn mới...',
        cancelBtn: 'Hủy',
        submitBtn: 'Đăng Kèo Ngay',
        submitting: 'Đang đăng kèo...',
        venueSuggest: 'Hoặc chọn nhanh quán cafe phổ biến:',
      },
      details: {
        title: 'Chi tiết kèo Board Game',
        hostInfo: 'Người tổ chức (Host)',
        timeLocation: 'Thời gian & Địa điểm',
        gameInfo: 'Thông tin Board Game',
        participants: 'Thành viên đã tham gia',
        noParticipants: 'Chưa có thành viên nào khác tham gia. Hãy là người đầu tiên!',
        joinAction: 'Tham gia kèo ngay',
        leaveAction: 'Rút khỏi kèo này',
        cancelAction: 'Hủy kèo này (Chỉ Host)',
        shareBtn: 'Chia sẻ kèo',
        addToCal: 'Thêm vào Google Calendar',
        copiedLink: 'Đã sao chép link sự kiện vào clipboard!',
        loginPrompt: 'Vui lòng đăng nhập để tạo hoặc tham gia kèo!',
      },
      empty: {
        title: 'Chưa tìm thấy kèo nào',
        desc: 'Hãy thử thay đổi từ khóa/bộ lọc hoặc là người đầu tiên lên kèo cho cộng đồng!',
        createFirst: 'Lên Kèo Đầu Tiên Ngay',
      },
      stats: {
        openMatches: 'Kèo đang mở',
        playersJoining: 'Lượt tham gia',
        popularPlaces: 'Địa điểm tụ tập',
      }
    }
  },
  en: {
    navbar: {
      home: 'Home',
      community: 'Community',
      events: 'Events',
      joinUs: 'Join Us',
      about: 'About',
      comingSoon: 'Coming soon',
      games: 'Games',
      news: 'News',
      shop: 'Market',
      logout: 'Logout',
      login: 'Login',
      profile: 'Profile',
      gameVault: 'Game Vault',
    },
    tour: {
      titleBar: 'quick_tour.exe',
      step: 'Step',
      skip: 'Skip',
      back: 'Back',
      next: 'Next',
      done: 'Start exploring',
      inMenu: 'Inside the ☰ menu',
      steps: {
        welcome: {
          title: 'Welcome to BoardMates',
          body: 'Looks like this is your first visit. Let us walk you through the navigation bar — it takes about 30 seconds.',
        },
        home: {
          title: 'Home',
          body: 'The starting point: what BoardMates is and what we are building.',
        },
        community: {
          title: 'Community',
          body: 'Where you find people who share your taste in board games. We are still building this one — it opens soon.',
        },
        events: {
          title: 'Events',
          body: 'Board Game Nights, workshops, tournaments. The schedule and sign-ups will live here.',
        },
        joinUs: {
          title: 'Join Us',
          body: 'We are recruiting our Core Team. Read each team’s job description and apply right here.',
        },
        about: {
          title: 'About',
          body: 'The vision, philosophy and roadmap of BoardMates — told across six faces of a die. At the bottom you will also find Youth+, the organisation we belong to.',
        },
        language: {
          title: 'Switch language',
          body: 'Move between Vietnamese and English whenever you like.',
        },
      },
    },
    footer: {
      brand: 'BoardMates',
      copyright: '© 2026 BoardMates. All rights reserved.',
      rights: 'All rights reserved.',
    },
    comingSoon: {
      badge: 'Coming soon',
      community: {
        title: 'Community',
        desc: 'Where you find people who share your interests, exchange ideas, and grow together through board games. We are building this part.',
      },
      events: {
        title: 'Events',
        desc: 'Board Game Nights, Workshops, Networking, Tournaments. The event calendar and registration will live here.',
      },
      cta: 'Want to help build it? Join the Core Team',
    },
    about: {
      title: 'About',
      intro: 'Roll the die to open each part of the BoardMates story.',
      diceHint: 'Click to roll',
      diceRolling: 'Rolling...',
      of: 'of',
      youth: {
        badge: 'Parent organisation',
        title: 'Youth+',
        body: 'Youth+ is a non-profit education and community ecosystem for young people in Vietnam. Its mission is to connect, guide and open up opportunities for them to grow through learning, hands-on experience and community projects.',
        modelLabel: 'Connection model',
        modelNodes: ['Youth', 'Youth', 'Mentor', 'Business'],
        modelNote: 'Building an environment where young people learn and create positive value together.',
        stats: [
          { value: '63K+', name: 'Facebook followers' },
          { value: '10+', name: 'Years running' },
        ],
        cta: 'Visit the Youth+ fanpage',
        ctaPending: 'Fanpage link coming soon',
      },
      sections: [
        {
          face: 1,
          kind: 'prose',
          label: 'Vision',
          body: 'BoardMates is a platform for connection and creativity through board games. Board games are only the starting point. What BoardMates wants to build is an ecosystem where people meet, share, learn, collaborate, and grow together.',
        },
        {
          face: 2,
          kind: 'lines',
          label: 'Philosophy',
          lines: [
            'Board games are the tool.',
            'People are the center.',
            'Connection is the value.',
            'Creativity is the goal.',
          ],
          closing: 'BoardMates exists to create meaningful connections through board games.',
        },
        {
          face: 3,
          kind: 'chain',
          label: 'Ecosystem',
          body: 'BoardMates is more than a community. We are building an ecosystem.',
          nodes: [
            'University Clubs',
            'Board Game Cafés',
            'Stores',
            'Publishers',
            'Creators',
            'Players',
          ],
        },
        {
          face: 4,
          kind: 'roadmap',
          label: 'Roadmap',
          phases: [
            { phase: 'Phase 1', name: 'Community', percent: 100 },
            { phase: 'Phase 2', name: 'Events', percent: 60 },
            { phase: 'Phase 3', name: 'Marketplace', percent: 20 },
            { phase: 'Phase 4', name: 'Platform', percent: 0 },
          ],
        },
        {
          face: 5,
          kind: 'groups',
          label: 'Goals',
          groups: [
            {
              name: 'Short-term',
              items: [
                'Introduce BoardMates',
                'Build the brand',
                'Recruit the Core Team',
                'Attract the first community',
                'Run the first event',
              ],
            },
            {
              name: 'Mid-term',
              items: [
                'Community management',
                'Event management',
                'Blog',
                'Resource Center',
                'Partner Network',
              ],
            },
            {
              name: 'Long-term',
              items: [
                'Marketplace',
                'Mobile App',
                'Community Hub',
                'Board Game Database',
                'Member Profile',
                'Gamification',
              ],
            },
          ],
        },
        {
          face: 6,
          kind: 'metrics',
          label: 'Success metrics',
          metrics: [
            { value: '1,000', name: 'Community members' },
            { value: '10', name: 'Core Team members' },
            { value: '20', name: 'Events' },
            { value: '50', name: 'Partners' },
            { value: '100', name: 'Marketplace listings' },
            { value: '10,000', name: 'Monthly visitors' },
          ],
        },
      ],
    },
    joinUs: {
      title: 'Join us',
      intro: 'BoardMates is still in its early days. We are looking for people who want to build a community from zero.',
      openPositions: 'Open positions',
      headcount: 'Openings',
      // Tieng Anh khong can don vi ("Openings: 2" du roi). De rong thi cho hien
      // so tu bo phan don vi — xem join-us/[slug]/page.js.
      headcountUnit: '',
      mission: 'Mission',
      responsibilities: 'Responsibilities',
      requirements: 'Requirements',
      benefits: 'Benefits',
      growthPath: 'Growth path',
      applyTitle: 'Sounds like you?',
      applyDesc: 'We care more about the will to build than about experience. If you want to start from zero with us, come talk to us.',
      applyCta: 'Apply',
      applyPending: 'The application channel is opening soon',
    },
    hero: {
      welcome: 'Welcome to',
      subtitle: 'A digital library of classic board games, preserving timeless memories and strategies.',
      explore: 'Explore Now',
      quickLinks: 'Quick Links',
      featured: 'Archiving is not just keeping items, but preserving an entire culture of play.',
      collector: 'Featured Collector',
    },
    home: {
      blog: {
        latest: 'Latest News',
        title: 'From our Blog',
        desc: 'We don\'t just archive games, we tell the stories behind them. Follow our latest articles on culture, history, and board game design.',
        viewAll: 'View all articles',
        readMore: 'Read more',
      },
      games: {
        newCollection: 'New Collection',
        title: 'Recent Projects',
        viewMore: 'View more',
        players: 'Players',
      },
      links: {
        blog: 'Blog',
        projects: 'Projects',
        info: 'Info',
        contact: 'Contact',
      }
    },
    auth: {
      welcomeBack: 'Welcome Back',
      startJourney: 'Start Your Journey',
      fullName: 'Full Name',
      email: 'Email',
      password: 'Password',
      confirmPassword: 'Confirm Password',
      forgotPassword: 'Forgot Password?',
      login: 'LOGIN',
      register: 'REGISTER',
      noAccount: "Don't have an account?",
      haveAccount: 'Already have an account?',
      registerNow: 'Register Now',
      loginNow: 'Login',
      agreeTerms: 'I agree to the terms and privacy policy.',
      processing: 'Processing...',
      authenticating: 'Authenticating...',
      mockRegisterSuccess: 'Mock registration successful! Redirecting...',
    },
    marketplace: {
      collection: '2024 COLLECTION',
      title: 'Marketplace',
      addGame: '+ Post Game',
      sortBy: 'Sort by:',
      newest: 'Newest',
      categories: 'Categories',
      price: 'Price Range',
      updating: 'UPDATING',
      maintenance: 'System maintenance in progress',
      underMaintenance: 'Feature under maintenance',
      memberOffer: 'Member Offer',
      memberDesc: 'Get 10% off your first order.',
      joinNow: 'Join Now',
    },
    profile: {
      myProfile: 'My Profile',
      editProfile: 'Edit Profile',
      listings: 'My Listings',
      rentals: 'Rentals',
      purchases: 'Purchases',
      noListings: "You haven't listed anything yet.",
      since: 'Member since',
      rating: 'Rating',
      addNew: '+ Add New',
      view: 'View',
      edit: 'Edit',
      remove: 'Remove',
      info: 'Personal Information',
      phone: 'Phone Number',
      address: 'Address',
      orders: {
        title: 'Order History',
        viewAll: 'View all',
        id: 'Order ID',
        date: 'Date',
        status: 'Status',
        total: 'Total',
        completed: 'Completed',
        cancelled: 'Cancelled',
      },
      favorites: {
        title: 'Favorite Games',
      }
    },
    modal: {
      addNew: 'Add New Boardgame',
      title: 'Game Title',
      players: 'Players',
      gameplay: 'Gameplay',
      description: 'Description',
      priceSell: 'Sell Price (VND)',
      priceRent: 'Rent Price (VND)',
      condition: 'Condition',
      type: 'Listing Type',
      imageUrl: 'Image',
      cancel: 'Cancel',
      submit: 'Post Game',
      submitting: 'Submitting...',
      mockSuccess: 'Post successful (Mock)',
      maintenanceMsg: 'This feature is under maintenance. Please try again later!',
    },
    events: {
      badge: 'Board Game Community',
      title: 'Events & Game Sessions',
      subtitle: 'Find like-minded players, join offline meetups, strategy workshops, and competitive tournaments.',
      createBtn: '+ Host / Create Event',
      searchPlaceholder: 'Search events by game title, venue, keywords...',
      tabs: {
        all: 'All Events',
        myHosted: 'Hosted by Me',
        myJoined: 'Events I Joined',
      },
      filterType: 'Event Type',
      filterCity: 'Location / City',
      filterStatus: 'Status',
      filterRadius: 'GPS Radius',
      gpsNearMe: '📍 Near Me (GPS)',
      gpsLocating: 'Locating...',
      gpsActive: 'GPS Active',
      gpsDistance: '{dist} km away',
      radiusAll: 'All Distances',
      radius5: '< 5 km',
      radius10: '< 10 km',
      radius25: '< 25 km',
      radius50: '< 50 km',
      allTypes: 'All Types',
      allCities: 'All Locations',
      allStatuses: 'All Statuses',
      types: {
        CASUAL: 'Casual Play / Match',
        TOURNAMENT: 'Tournament',
        WORKSHOP: 'Workshop / Learn & Play',
        NIGHT: 'Board Game Night',
      },
      statuses: {
        OPEN: 'Open Slots',
        FULL: 'Full',
        ONGOING: 'In Progress',
        COMPLETED: 'Completed',
        CANCELLED: 'Cancelled',
      },
      skillLevels: {
        ALL: 'All Levels Welcome',
        BEGINNER: 'Beginner Friendly',
        INTERMEDIATE: 'Intermediate',
        EXPERT: 'Expert / Competitive',
      },
      card: {
        host: 'Host:',
        slots: 'Slots:',
        full: 'Full',
        fee: 'Fee:',
        free: 'Free / Split drinks',
        joinBtn: 'Join Match',
        joinedBtn: 'Joined',
        viewDetails: 'View Details',
        spotsLeft: '{count} spots left',
        away: '{dist} km away',
      },
      modal: {
        createTitle: 'Host a Game Session / Event',
        titleLabel: 'Event / Session Title',
        titlePlaceholder: 'e.g., Weekend Catan Chill Session, Scythe for Beginners...',
        typeLabel: 'Event Format',
        gameLabel: 'Board Game Name',
        gamePlaceholder: 'e.g., Catan, Splendor, Root, Dune: Imperium...',
        imageLabel: 'Cover Image URL',
        imagePlaceholder: 'https://images.unsplash.com/...',
        dateLabel: 'Start Date & Time',
        endDateLabel: 'End Date & Time (Optional)',
        locationLabel: 'Venue / Cafe Name',
        locationPlaceholder: 'e.g., The Mind Cafe, Cube Cafe, Private Venue...',
        addressLabel: 'Specific Address',
        addressPlaceholder: 'e.g., 284/41 Ly Thuong Kiet, Dist 10...',
        cityLabel: 'City / Region',
        maxParticipantsLabel: 'Max Players (Including Host)',
        feeLabel: 'Entry Fee (VND)',
        feeHint: 'Enter 0 for Free or Split Drinks',
        skillLevelLabel: 'Skill Level Required',
        descLabel: 'Description & House Rules',
        descPlaceholder: 'Notes about the session, game rules, who brings games, tutorial for beginners...',
        cancelBtn: 'Cancel',
        submitBtn: 'Publish Event',
        submitting: 'Publishing...',
        venueSuggest: 'Or quickly pick a popular cafe:',
      },
      details: {
        title: 'Board Game Event Details',
        hostInfo: 'Host Information',
        timeLocation: 'Time & Location',
        gameInfo: 'Board Game Info',
        participants: 'Joined Players',
        noParticipants: 'No other players joined yet. Be the first one!',
        joinAction: 'Join this Match',
        leaveAction: 'Leave this Match',
        cancelAction: 'Cancel Event (Host only)',
        shareBtn: 'Share Event',
        addToCal: 'Add to Google Calendar',
        copiedLink: 'Event link copied to clipboard!',
        loginPrompt: 'Please login to host or join events!',
      },
      empty: {
        title: 'No Events Found',
        desc: 'Try adjusting your filters or be the first to host an event for the community!',
        createFirst: 'Host the First Event',
      },
      stats: {
        openMatches: 'Open Sessions',
        playersJoining: 'Participants',
        popularPlaces: 'Active Venues',
      }
    }
  }
};
